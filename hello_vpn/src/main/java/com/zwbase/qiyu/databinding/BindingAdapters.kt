...
import com.zwbase.qiyu.activity.pay.PayAdapter
...
object BindingAdapters {
    ...
    fun <K> setItems(view: RecyclerView,
                     oldList: List<K>?, oldLayoutId: Int,
                     @Suppress("UNUSED_PARAMETER") oldRowConfigurationHandler: PayAdapter.RowConfigurationHandler<*, *>?,
                     newList: List<K>?, newLayoutId: Int,
                     newRowConfigurationHandler: PayAdapter.RowConfigurationHandler<*, *>?) {
        if (view.layoutManager == null)
//            view.layoutManager = LinearLayoutManager(view.context, RecyclerView.VERTICAL, false)
            view.layoutManager = GridLayoutManager(view.context, 2, RecyclerView.VERTICAL, false)

        if (view.itemDecorationCount<=0){
            view.addItemDecoration(MarginItemDecortion())
        }

        if (oldList === newList && oldLayoutId == newLayoutId)
            return
        // The ListAdapter interface is not generic, so this cannot be checked.
        @Suppress("UNCHECKED_CAST") var adapter = view.adapter as? PayAdapter<K>?
        ...
        if (newList == null || newLayoutId == 0)
            return
        if (adapter == null) {
            adapter = PayAdapter(view.context, newLayoutId, newList)
