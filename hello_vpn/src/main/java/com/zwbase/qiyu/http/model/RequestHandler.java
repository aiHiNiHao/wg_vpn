...
import com.zwbase.qiyu.http.json.BooleanTypeAdapter;
import com.zwbase.qiyu.http.json.DoubleTypeAdapter;
import com.zwbase.qiyu.http.json.FloatTypeAdapter;
import com.zwbase.qiyu.http.json.IntegerTypeAdapter;
import com.zwbase.qiyu.http.json.ListTypeAdapter;
import com.zwbase.qiyu.http.json.LongTypeAdapter;
import com.zwbase.qiyu.http.json.StringTypeAdapter;
    ...
    public Object requestSucceed(LifecycleOwner lifecycle, Response response, Type type) throws Exception {
        ...
        if (String.class.equals(type)) {
            ...
            if (result instanceof HttpData) {
                ...
                if (TextUtils.equals(model.getCode() , "0000")) {
                    // 代表执行成功
                    return result;
                } else {
                    // 代表执行失败
                    throw new CodeException("error: code->" + model.getCode(), model.getCode());
                }
            }
        }
        return result;
